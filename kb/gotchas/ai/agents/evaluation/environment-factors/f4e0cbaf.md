...
SOURCING SCOPE. Every figure and quoted string above was confirmed by a second verbatim-demanding fetch. One thing the check found ABSENT: the post does not list all twelve factors in one place, so the full taxonomy is not reproduced here and should not be reconstructed from the examples it names in passing.
