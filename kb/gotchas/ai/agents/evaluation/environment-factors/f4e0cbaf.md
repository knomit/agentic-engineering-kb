...
SOURCING SCOPE, AND A CORRECTION MADE IN THE SAME SESSION THIS FACT WAS WRITTEN. Every figure and quoted string above was confirmed by a second verbatim-demanding fetch. That check found one thing ABSENT: the post does not list all twelve factors in one place. An earlier draft of this fact nonetheless named three example strategic factors and four non-strategic ones, taken from the first open-ended extraction and never verified — while simultaneously warning the reader not to reconstruct the taxonomy from examples. They have been removed. Only the two named most-influential non-strategic factors survive, because those are quoted verbatim from the source. If you need the full twelve, go to the post; do not take them from here.
